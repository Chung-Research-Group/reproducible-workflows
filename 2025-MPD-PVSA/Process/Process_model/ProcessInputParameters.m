function InputParams = ProcessInputParameters(process_vars, N)
...
%% State all input parameters for the simulation
    ...
    mu         = 1.13e-5                ;   % Viscosity of gas [Pa*s]
    epsilon    = 0.40                   ;   % Void fraction
    D_m        = 1.2995e-5              ;   % Molecular diffusivity [m^2/s]
    K_z        = 0.09                   ;   % Thermal conduction in gas phase [W/m/k]
    C_pg       = 35.8                   ;   % Specific heat of gas [J/mol/k]
    C_pa       = 35.8                   ;   % Specific heat of adsorbed phase [J/mol/k]
    MW_CO      = 0.044                  ;   % Molecular weight of CO [kg/mol]
    MW_CO2     = 0.016                  ;   % Molecular weight of CO2 [kg/mol]
    ...
    ro_s        = 1556.5                ;   % Density of the adsorbent [kg/m^3]
    r_p         = 5e-3                  ;   % Radius of the pellets [m]
    C_ps        = 750                   ;   % Specific heat capacity of the adsorbent [J/kg/K]
    q_s         = 7.4                   ;   % Molar loading scaling factor [mol/kg]
    q_s0        = q_s*ro_s              ;   % Molar loading scaling factor [mol/m^3]
    k_CO_LDF    = 0.1600                ;   % Mass transfer coefficient for CO [1/s]
    k_CO2_LDF   = 0.2000                ;   % Mass transfer coefficient for CO2 [1/s]
    
    % Isotherm parameters
    deltaU     = [-35217.6, -19017.1]    ; % Heat of adsorption for high (n_g)

    % MPD parameters
    df = readtable('NVTpW_summary.csv');

    selectedRow = df(strcmp(df.Name, 'AFG-1'), :);
    
    Structure = selectedRow.Name{1};  % 'APC-0'
    NumOfCom1 = selectedRow.CO2(1);  % component1 max
    NumOfCom2 = selectedRow.CH4(1);  % component2 max
    Cell_a = selectedRow.a(1);
    Cell_b = selectedRow.b(1);
    Cell_c = selectedRow.c(1);
    Unit = selectedRow.Unit(1);

    LN_Pi_filename = sprintf('LN_Pi_%s.csv', Structure);
    Ene_filename = sprintf('Ene_%s.csv', Structure);
    state_rd_filename = sprintf('state_rd_%s.csv', Structure);
    
    LN_Pi_table = readtable(LN_Pi_filename);
    Ene_table = readtable(Ene_filename);
    state_rd_table = readtable(state_rd_filename);

    LN_Pi = table2array(LN_Pi_table);
    Ene = table2array(Ene_table);
    state_rd = table2array(state_rd_table);
...
%% Distribute the values to the necessary variables
    ...
    MPDParams = [NumOfCom1, NumOfCom2, Cell_a, Cell_b, Cell_c, Unit] ;
%   
%% Combine all lists into one variable of cells that can easily be passed
    InputParams{1} = Params          ;
    InputParams{2} = MPDParams  ;
    InputParams{3} = Times           ;
    InputParams{4} = LN_Pi ;
    InputParams{5} = Ene ; 
    InputParams{6} = state_rd ;
